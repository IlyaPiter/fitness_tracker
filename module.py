...
@dataclass
class InfoMessage:
    """Информационное сообщение о тренировке и мой первый dataclass."""
    ...
    speed: float
    calories: float
    template = ('Тип тренировки: {training_type}; Длительность: '
                '{duration:.3f} ч.; Дистанция: {distance:.3f} км;'
                ' Ср. скорость: {speed:.3f} км/ч; Потрачено ккал: '
                '{calories:.3f}.')

    def get_message(self) -> str:
        return (self.template.format(training_type=self.training_type,
...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        calories_spent = ((self.CALORIES_MEAN_SPEED_MULTIPLIER
                          * self.get_mean_speed()
...
class SportsWalking(Training):
    ...
    CALORIES_MEAN_HEIGHT_SPEED_MULTIPLIER: float = 0.029
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        calories_spent = ((self.CALORIES_MEAN_WEIGHT_MULTIPLIER * self.weight
                          + ((self.get_mean_speed() * self.KMH_IN_MS) ** 2
                             / (self.height / self.SM_IN_M))
                          * self.CALORIES_MEAN_HEIGHT_SPEED_MULTIPLIER
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        calories_spent = ((self.get_mean_speed()
...
def read_package(workout_type: str,
                 data: list[int, int, int, int, int]) -> Training:
    """Прочитать данные полученные от датчиков."""
    training_var: dict = {'SWM': Swimming, 'RUN': Running,
                          'WLK': SportsWalking}
    if workout_type not in training_var:
        raise ValueError('Неправильный тип тренировки')
        return None
    else:
        training = training_var[workout_type](*data)
    return training
